...
function RePrintOR() {
  ...
  let schemeValidations = Yup.object().shape({
    [customerName.name]: customerName.isRequired ? Yup.string().required(customerName.errorMsg) : Yup.string().notRequired()
  });
  const schemeInitialValues = {
    [customerName.name]: customerName.defaultValue
  };
  useEffect(() => { document.getElementsByName(customerName.name)[0].focus() }, []);
      ...
      columns: [
        {
          ...
          Cell: ({ row }) => {
            return <Radio
              ...
              name="radio-buttons" value={row.original}
  ...
  useEffect(() => { (customerRequest.keywords != "") && getCustomerList() }, [customerRequest.skipCount, customerRequest.recordsPerPage]);
    ...
    <DashboardLayout>
      ...
      <MDBox py={3}>
        <Grid container spacing={3}>
          ...
          <Grid item xs={12}>
            <Card>
              <MDBox p={3} lineHeight={1}>
                  ...
                  <Grid item xs={12}>
                    ...
                    >
                      {({ values, errors, touched }) => {
                        ...
                        const isValifForm = () => {
                          return checkingSuccessInput(customerNameV, errors.customerName) ? true : false;
                        };
                          ...
                          > {/* pb={3} px={3} */}
                            <Grid container spacing={3}>
                              <Grid item xs={12} sm={9}>
                                <FormField type={customerName.type} label={customerName.label + (customerName.isRequired ? ' ⁽*⁾' : '')} name={customerName.name} value={customerNameV} placeholder={customerName.placeholder}
                                  error={errors.customerName && touched.customerName}
                                  success={checkingSuccessInput(customerNameV, errors.customerName)}
                                  onKeyUp={(e) => setCustomerRequest((prevState) => ({
                                    ...prevState,
                ...
                {(customerResponse.rowData.length > 0) && <Grid container alignItems="center" pt={3}>
                  <Grid item xs={12}>
                    <MDBox mb={1}>
                      <MDTypography variant="h5">
                        Search Result
                    ...
                    <MDBox mb={2}>
                      <MDTypography variant="body2" color="text">
                        Customers related to search "{customerRequest.keywords}".
                      </MDTypography>
                    </MDBox>
                  </Grid>
                  <Grid item xs={12}>
                    <DataTable
                      ...
                      totalRows={customerResponse.totalRows} totalPages={customerResponse.totalPages}
                      recordsPerPage={customerRequest.recordsPerPage} skipCount={customerRequest.skipCount}
                    ...
                    <MDBox pt={1} pb={1} px={3}>
                      <Grid item xs={12}>
                        <MDBox display="flex" flexDirection={{ xs: "column", sm: "row" }} justifyContent="center">
                          <MDBox ml={{ xs: 0, sm: 1 }} mt={{ xs: 1, sm: 0 }}>
                            <MDButton type="button" variant="gradient" color="info" sx={{ height: "100%" }}
                              onClick={() => getOfficialReceiptList(selectedUnit.unitDataId)} disabled={!selectedUnit}
                            >
                              Show this Unit {selectedUnit && <sup>&nbsp;({selectedUnit.unitCode}-{selectedUnit.unitNo})</sup>}
                            </MDButton>
                          </MDBox>
                        </MDBox>
                      </Grid>
          ...
          {(officialReceiptData.rowData.length > 0) && <Grid item xs={12}>
            <Card>
              <MDBox p={3} lineHeight={1}>
                <Grid container alignItems="center">
                  <Grid item xs={12}>
                    ...
                    <MDBox mb={2}>
                      <MDTypography variant="body2" color="text">
                        Official receipts of {selectedUnit.projectName} {selectedUnit.unitName} {selectedUnit.unitCode} {selectedUnit.unitNo} on behalf of {selectedUnit.customerName}.
                      </MDTypography>
                    </MDBox>
                  </Grid>
                  <Grid item xs={12}>
                    <DataTable
                      ...
                      canEntriesPerPage entriesPerPage={{ defaultValue: customerRequest.recordsPerPage }} canSearch
                    />
                  </Grid>
                </Grid>
              </MDBox>
