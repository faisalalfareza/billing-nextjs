...
// Data
import UploadDataElectric from "./components/UploadDataElectric";
import ElectricRowActions from "./components/ElectricRowActions";
import EditDataElectric from "./components/EditDataElectric";
import SiteDropdown from "../../../pagesComponents/dropdown/Site";
import { Block } from "notiflix/build/notiflix-block-aio";

export default function ElectricReading() {
  ...
  const form = {
    formId: "electric-reading-filter",
    formField: {
      project: {
        name: "project",
        label: "Project",
  ...
  const electricReadingBlockLoadingName = "block-electric-reading";
  const fetchData = async () => {
    Block.standard(
      `.${electricReadingBlockLoadingName}`,
      `Getting Electric Reading Data`
    ),
      setLoading(true);

    const { scheme, keywords, recordsPerPage, skipCount } = customerRequest;
    let response = await fetch(
      "/api/transaction/electric/getelectricreadinglist",
      {
        method: "POST",
        body: JSON.stringify({
          accessToken: accessToken,
    ...
    if (response.error)
      ...
      setTimeout(() => {
        ...
        element.href = "#electric-reading";
        element.click();
      }, 0);
    }

    Block.remove(`.${electricReadingBlockLoadingName}`), setLoading(false);
  };
  ...
  return (
    <DashboardLayout>
      ...
      <MDBox mt={3.5} id="electric-reading">
        <MDBox
          display="flex"
          justifyContent="flex-end"
        ...
        >
          <MDBox display="flex">
            ...
            <MDBox ml={1}>
              ...
              >
                <Icon>add</Icon>&nbsp; Add New Electric Reading
              </MDButton>
            </MDBox>
          </MDBox>
        </MDBox>
        <Card className={electricReadingBlockLoadingName}>
          <MDBox>
            <Grid container alignItems="center">
              <Grid item xs={12} mb={1}>
                <DataTable
                  title="Electric Reading List"
                  description="Electric Reading Data"
