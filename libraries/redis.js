...
export const isRedis = (process.env.IS_REDIS == "true") || false;
const redisIO = isRedis && createRedisInstance();
...
function getRedisConfiguration() {
    return {
        ...
        port: process.env.REDIS_PORT || 6379, // Optional. Default to "6379"
        username: process.env.REDIS_USERNAME || "default", // Optional if not set on redis server
        password: process.env.REDIS_PASSWORD, // Optional if not set on redis server
        database: process.env.REDIS_DB || 0, // Optional. Default to "0". Available 16 databases (0 - 15)
    };
}
...
export function createRedisInstance(config = getRedisConfiguration()) { 
    try {
        const options = {
            ...
            */
            showFriendlyErrorStack: (process.env.CURRENT_ENV !== "production"),
            ...
            */
            retryStrategy: (times) => {
                if (times > options.maxRetriesPerRequest) throw new Error(`[Redis] Could not connect after ${times} attempts`);
